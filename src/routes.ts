...
import { resolveSafePath } from "@vibecontrols/plugin-sdk/host/fs-safety";
import { denyNonLocalMutation } from "@vibecontrols/plugin-sdk/host/request-security";

import type { SessionProvider } from "./provider.js";
import type { SessionServiceRegistry, TunnelProvider } from "./types.js";

// ── Provider name resolution ────────────────────────────────────────────
// Translates a backend session type into the
// pluginName each provider registers under, by matching the hint against the
// session providers actually REGISTERED in the runtime ServiceRegistry. The
// agent holds no provider knowledge — adding a new session backend is just a
// new provider plugin (declared by the session meta's `metaProviders`); it
// registers itself and this resolver finds it.
...
function resolveSessionProviderPluginName(
  ...
  const names = registry
    .listProvidersForType("session")
    .map((p) => p.pluginName);
  // Exact pluginName match first, then a registered
  // provider whose name contains the backend hint.
  return (
...
//
// Per Plugin Contract v2 this state must be per-instance (one map per
// createPlugin(ctx) invocation). It's instantiated inside `createRoutes`
// and threaded into helpers explicitly — never hung off the module.
type SessionProviderMap = Map<string, string>;
...
function getSessionProvider(
  ...
  if (providerHint) {
    const pluginName = resolveSessionProviderPluginName(registry, providerHint);
    if (pluginName) {
...
async function getProviderForSession(
  ...
  if (remembered) {
    const pluginName = resolveSessionProviderPluginName(registry, remembered);
    if (pluginName) {
...
export function createSessionRoutes(serviceRegistry: SessionServiceRegistry) {
  // Per-instance state — moves the previous module-level Map into the
  // closure so multi-profile daemons (Phase 3) get isolated routing
  // tables instead of one shared global.
  const sessionProviderMap: SessionProviderMap = new Map<string, string>();
